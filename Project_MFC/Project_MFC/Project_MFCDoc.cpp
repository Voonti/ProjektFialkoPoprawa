...
void CProjectMFCDoc::Serialize(CArchive& ar)
{
	if (ar.IsStoring())
	{
		...
		for (size_t i = 0; i < capacity; ++i)
		{
			...
			ar.Write(&pTabTemp[i].pointSize, sizeof(pTabTemp[i].pointSize));
			ar.Write(&pTabTemp[i].fontSize, sizeof(pTabTemp[i].fontSize));
	...
	else
	{
		...
		for (size_t i = 0; i < capacity; ++i)
		{
			...
			ar.Read(&myPointObj.pointSize, sizeof(myPointObj.pointSize));
			ar.Read(&myPointObj.fontSize, sizeof(myPointObj.fontSize));
			...
			if (nameLength > 0)
			{
				...
				myPointObj.name[nameLength] = '\0'; // Null-terminate the string
			}
			else
			{
